...
export const AgentCard: React.FC<AgentCardProps> = ({ agent, className }) => {
    return (
        <Link href={`/agents/${agent.slug}`} className={cn("group block", className)}>
            <Card className="overflow-hidden border-stone/10 group-hover:border-lime/30 group-hover:shadow-soft transition-all duration-500 bg-white rounded-[2rem] flex flex-col h-full hover:-translate-y-2">
                    ...
                    <div className="p-8 bg-warm/30 flex flex-col items-center justify-center border-b border-stone/5 relative overflow-hidden">
                        <div className="absolute top-4 right-4 flex flex-col gap-2">
                            <Button
                                variant="ghost"
                                size="icon"
                                className="w-10 h-10 rounded-full bg-white/80 backdrop-blur-sm shadow-sm text-stone/40 hover:text-lime transition-colors"
                                onClick={(e) => {
                                    e.preventDefault();
                                    e.stopPropagation();
                                }}
                        ...
                        <div className="w-24 h-24 rounded-3xl bg-lime/10 flex items-center justify-center mb-4 border-4 border-white shadow-lg overflow-hidden group-hover:scale-110 transition-transform duration-500">
                            {agent.headshot_url ? (
                                <Image
                                ...
                                <div className="text-lime font-display font-black text-3xl">BHQ</div>
                            )}
                        </div>
                    ...
                    <div className="p-8 flex-1 space-y-6 flex flex-col">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {agent.specialisations?.slice(0, 3).map(spec => (
                                <Badge key={spec} variant="secondary" className="bg-lime/5 text-lime border-transparent font-bold px-3 py-1 text-[10px] uppercase tracking-wider rounded-lg">
                                    {spec}
                                </Badge>
                            ))}
                        </div>
                        ...
                        <div className="flex items-center justify-between pt-6 border-t border-stone/5 mt-auto">
                            <div className="flex items-center gap-2 text-xs font-bold text-midnight">
                                <div className="w-6 h-6 bg-lime/10 rounded-lg flex items-center justify-center">
                                    <Award className="w-3.5 h-3.5 text-lime" />
                                </div>
                                {agent.years_experience || 10}+ Years
                            </div>
                            <Button className="bg-midnight group-hover:bg-lime text-white font-black h-10 px-6 rounded-xl transition-all shadow-lg flex items-center gap-2">
                                View Profile
