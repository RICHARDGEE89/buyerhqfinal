-- ============================================
-- BUYERHQ.COM.AU - FINAL DATABASE FIX
-- Fixes RLS recursion error + Complete setup
-- ============================================
...
-- 3. CREATE FIXED RLS POLICIES (NO RECURSION)
...
-- Agents table - Public read, owner write
...
-- Saved agents - Buyer ownership
CREATE POLICY "Buyers can manage their saved agents" ON public.saved_agents 
  FOR ALL USING (auth.uid() = buyer_id);

-- Match results - Buyer ownership
CREATE POLICY "Buyers can manage match results" ON public.match_results 
  FOR ALL USING (auth.uid() = buyer_id);

-- 4. CREATE AUTO-SYNC TRIGGER
